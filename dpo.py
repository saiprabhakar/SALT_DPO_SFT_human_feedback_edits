# Parts of the code are adapted from the Hugging Face TRL library
...
class BaseTrainer(Trainer):
    ...
    def __init__(
        ...
        if not is_peft_available() and self.peft_config is not None:
            raise ValueError(
                "PEFT is not installed and you passed a `peft_config` in the trainer's kwargs, please install it to use the PEFT models"
            )
        elif is_peft_available() and self.peft_config is not None:
...
class DPOTrainer(BaseTrainer):
    ...
    def get_batch_metrics(
        ...
        metrics[f"{prefix}rewards/rejected"] = (
            rejected_rewards[rewards_mask].cpu().numpy().mean()
        )
        metrics[f"{prefix}rewards/accuracies"] = (
            reward_accuracies[rewards_mask].cpu().numpy().mean()
        )
        metrics[f"{prefix}rewards/margins"] = (
            (chosen_rewards - rejected_rewards)[rewards_mask].cpu().numpy().mean()
        )
        metrics[f"{prefix}logps/rejected"] = (
            policy_rejected_logps[rewards_mask].detach().cpu().numpy().mean()
        )
        metrics[f"{prefix}logps/chosen"] = (
            policy_chosen_logps[rewards_mask].detach().cpu().numpy().mean()
        )
        metrics[f"{prefix}logits/rejected"] = (
            policy_rejected_logits[rewards_mask].detach().cpu().numpy().mean()
        )
        metrics[f"{prefix}logits/chosen"] = (
            policy_chosen_logits[rewards_mask].detach().cpu().numpy().mean()
        )

        if train_eval == "eval":
            policy_output_decoded, reference_output_decoded = self.get_batch_samples(
                model, batch
            )
            ...
            eval_dict = ngram_eval.run_all_evaluation(
                batch["chosen_response_only"], policy_output_decoded
            )

            eval_dict = {
                "eval_metrics_policy_" + k: round(v, 4) for k, v in eval_dict.items()
            }

            # eval generated summary for ref model
            ref_eval_dict = ngram_eval.run_all_evaluation(
                batch["chosen_response_only"], reference_output_decoded
            )
            ref_eval_dict = {
                "eval_metrics_ref_" + k: round(v, 4) for k, v in ref_eval_dict.items()
            }
    ...
    def get_batch_samples(
        ...
        policy_output = pad_to_length(
            policy_output, self.generate_max_length, self.tokenizer.pad_token_id
        )
        reference_output = pad_to_length(
            reference_output, self.generate_max_length, self.tokenizer.pad_token_id
        )
...
class SALTTrainer(BaseTrainer):
    ...
    def sequence_alignment(
        ...
        for S_generated, S_edited in zip(
            rejected_input_ids.tolist(), chosen_input_ids.tolist()
        ):
            # Create the instance
            alignment = needle.NeedlemanWunsch(
                S_generated, S_edited
            )  # needle.NeedlemanWunsch, smith.SmithWaterman
            ...
            w_generated, w_edited = needle.get_position_status(
                al_generated,
                al_edited,
        ...
        batch["rejected_salt_weight"] = torch.tensor(
            rejected_salt_weight, dtype=torch.float32
        ).to(self.accelerator.device)
        batch["chosen_salt_weight"] = torch.tensor(
            chosen_salt_weight, dtype=torch.float32
        ).to(self.accelerator.device)
    ...
    def _get_batch_chosen_salt_logps(
        ...
        likelihood_weight = torch.where(
            labels_weight > zero_for_here, labels_weight, zero_for_here
        )
    ...
    def _get_batch_rejected_salt_logps(
        ...
        if calculate_liklihood_token_log_prob:
            likelihood_per_token_logps = torch.gather(
                logits.log_softmax(-1), dim=2, index=labels.unsqueeze(2)
            ).squeeze(2)
            ...
            if average_log_prob:
                ...
                if likelihood_token_num == 0:
                    ...
                    likelihood_per_token_logps = (
                        likelihood_per_token_logps.sum(-1) / likelihood_token_num
                    ).mean()
            else:
        ...
        unlikelihood_weight = torch.where(
            labels_weight < zero_for_here, -1 * labels_weight, zero_for_here
        )

        unlikelihood_weight = unlikelihood_weight * loss_mask
        unlikelihood_per_token_logps = (
            unlikelihood_per_token_logps * unlikelihood_weight
        )
        ...
        if average_log_prob:
            ...
            if unlikelihood_token_num == 0:
                ...
                unlikelihood_per_token_logps = (
                    unlikelihood_per_token_logps.sum(-1) / unlikelihood_token_num
                ).mean()
        else:
    ...
    def concatenated_forward(
        ...
        chosen_salt_logps = self._get_batch_chosen_salt_logps(
            ...
            concatenated_batch["concatenated_salt_weight"][
                : batch["chosen_input_ids"].shape[0]
            ],
            average_log_prob=True,
        )
    ...
    def get_batch_metrics(
        ...
        pi_logratios = policy_chosen_logps - policy_rejected_logps
        rewards_mask = pi_logratios != 0
        losses = (-1) * self.omega1 * policy_chosen_salt_logps + (
            -1
        ) * self.omega2 * policy_rejected_salt_logps
        ...
        if train_eval == "eval":
            policy_output_decoded = self.get_batch_samples(model, batch)
